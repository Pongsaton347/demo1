  ...
  <div :class="widgetClasses" class="card">
    ...
    <div class="card-header border-0 pt-5">
      <h3 class="card-title align-items-start flex-column">
        <span class="card-label fw-bold fs-3 mb-1">Experience</span>
      </h3>
      <!--begin::Card toolbar-->
    ...
    <div class="card-body pt-0 pb-5">
      ...
      >
        <template v-slot:employeeExperienceSequence="{ row: items }">
          {{ items.employeeExperienceSequence }}
        </template>
        <template v-slot:companyName="{ row: items }">
          {{ items.companyName }}
        </template>
        <template v-slot:position="{ row: items }">
          {{ items.position }}
        </template>
        <template v-slot:reasonToLeave="{ row: items }">
          {{ items.reasonToLeave }}
        </template>
...
export default defineComponent({
  ...
  methods: {
    async created() {
      try {
        await axios
          .get("https://localhost:7176/api/EmployeeExperience/1576")
          .then((response) => {
  ...
  setup() {
    const tableHeader = ref([
      {
        columnName: "#",
        columnLabel: "employeeExperienceSequence",
        sortEnabled: false,
      },
      {
      ...
      {
        columnName: "reasonToLeave",
        columnLabel: "reasonToLeave",
        sortEnabled: false,
      },
      {
        columnName: "Date",
